...
impl LobbyInfos {
    pub async fn from_lobbies(lobbies: Arc<Mutex<Lobbies>>) -> Self {
...
impl LobbyInfo {
    pub async fn from_lobby(lobby: Arc<Mutex<Lobby>>) -> Self {
...
mod tests {
    ...
    async fn from_lobby_with_lobby_return_lobby_info() -> Result<(), Box<dyn Error + Send + Sync>> {
        let lobby = Arc::new(Mutex::new(Lobby::new(0, 4)));
    ...
    async fn from_lobby_with_a_player_in_lobby_lobby_info_current_players_should_be_one(
        ...
        lobby
            .lock()
            .await
            .add_player(Arc::new(Mutex::new(Player::new(0, String::from("test")))))
            .await?;
        let lobby_info = LobbyInfo::from_lobby(lobby).await;
    ...
    async fn from_lobbies_with_lobbies_has_one_lobby_return_lobby_infos(
        ...
        let lobbies = Arc::new(Mutex::new(Lobbies::new()));
        lobbies.lock().await.create_lobby(4).await;
        let lobby_infos = LobbyInfos::from_lobbies(lobbies).await;
